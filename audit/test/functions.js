// ETH/USD 21 Feb 2018 08:26 AEDT from CMC and https://ethgasstation.info/
var ethPriceUSD = 917.45;
...
function printTokenContractDetails() {
  ...
  if (tokenContractAddress != null && tokenContractAbi != null) {
    ...
    console.log("RESULT: token.INITIAL_SUPPLY=" + contract.INITIAL_SUPPLY() + " " + contract.INITIAL_SUPPLY().shift(-decimals));
    console.log("RESULT: token.maxSupply=" + contract.maxSupply() + " " + contract.maxSupply().shift(-decimals));
...
// -----------------------------------------------------------------------------
// TestTokenFallback Contract
// -----------------------------------------------------------------------------
var testTokenFallbackContractAddress = null;
var testTokenFallbackContractAbi = null;

function addTestTokenFallbackContractAddressAndAbi(address, testTokenFallbackAbi) {
  testTokenFallbackContractAddress = address;
  testTokenFallbackContractAbi = testTokenFallbackAbi;
}

var testTokenFallbackFromBlock = 0;
function printTestTokenFallbackContractDetails() {
  console.log("RESULT: testTokenFallbackContractAddress=" + testTokenFallbackContractAddress);
  if (testTokenFallbackContractAddress != null && testTokenFallbackContractAbi != null) {
    var contract = eth.contract(testTokenFallbackContractAbi).at(testTokenFallbackContractAddress);
    console.log("RESULT: testTokenFallback.owner=" + contract.owner());
    console.log("RESULT: testTokenFallback.requireFlag=" + contract.requireFlag());
    console.log("RESULT: testTokenFallback.successFlag=" + contract.successFlag());

    var latestBlock = eth.blockNumber;
    var i;

    var logTokenFallbackEvents = contract.LogTokenFallback({}, { fromBlock: testTokenFallbackFromBlock, toBlock: latestBlock });
    i = 0;
    logTokenFallbackEvents.watch(function (error, result) {
      console.log("RESULT: LogTokenFallback " + i++ + " #" + result.blockNumber +
          " msgSender=" + result.args.msgSender +
          " from=" + result.args.from +
          " amount=" + result.args.amount.shift(-18) +
          " data='" + web3.toAscii(result.args.data) + "'");
    });
    logTokenFallbackEvents.stopWatching();

    var logReceiveApprovalEvents = contract.LogReceiveApproval({}, { fromBlock: testTokenFallbackFromBlock, toBlock: latestBlock });
    i = 0;
    logReceiveApprovalEvents.watch(function (error, result) {
      console.log("RESULT: LogReceiveApproval " + i++ + " #" + result.blockNumber +
          " msgSender=" + result.args.msgSender +
          " amount=" + result.args.amount.shift(-18) +
          " token=" + result.args.token +
          " data='" + web3.toAscii(result.args.data) + "'");
    });
    logReceiveApprovalEvents.stopWatching();

    testTokenFallbackFromBlock = latestBlock + 1;
  }
}